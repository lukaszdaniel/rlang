...
static inline r_type r_typeof(r_obj* x) {
...
static r_unused r_obj* _r_placeholder = NULL;
...
static inline r_obj* r_type_as_string(r_type type) {
    return Rf_type2str_nowarn(type);
}
static inline r_obj* r_type_as_character(r_type type) {
    r_obj* str = KEEP(r_type_as_string(type));
    r_obj* out = Rf_ScalarString(str);
    return FREE(1), out;
}
static inline const char* r_type_as_c_string(r_type type) {
    return CHAR(Rf_type2str_nowarn(type));
}

static inline r_type r_c_str_as_r_type(const char* type) {
    return (r_type) Rf_str2type(type);
}
r_type r_chr_as_r_type(r_obj* type);

static inline bool r_is_symbolic(r_obj* x) {
