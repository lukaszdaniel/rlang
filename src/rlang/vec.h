...
static inline void* r_vec_begin0(r_type type, r_obj* x) {
    switch (type) {
    case R_TYPE_logical:
...
static inline void* r_vec_begin(r_obj* x) {
    return r_vec_begin0(r_typeof(x), x);
}

static inline const void* r_vec_cbegin0(r_type type, r_obj* x) {
    switch (type) {
    case R_TYPE_logical:
...
static inline const void* r_vec_cbegin(r_obj* x) {
    return r_vec_cbegin0(r_typeof(x), x);
}

static inline int r_vec_elt_sizeof0(r_type type) {
    switch (type) {
    case R_TYPE_logical:
        return sizeof(int);
    case R_TYPE_integer:
...
static inline int r_vec_elt_sizeof(r_obj* x) {
...
static inline r_obj* r_list_get(r_obj* x, r_ssize i) {
...
static inline void r_list_poke(r_obj* x, r_ssize i, r_obj* y) {
...
static inline r_obj* r_alloc_vector(r_type type, r_ssize n) {
...
static inline r_obj* r_alloc_logical(r_ssize n) {
#if R_VERSION >= R_Version(4, 6, 0)
...
static inline r_obj* r_alloc_integer(r_ssize n) {
#if R_VERSION >= R_Version(4, 6, 0)
...
static inline r_obj* r_alloc_double(r_ssize n) {
#if R_VERSION >= R_Version(4, 6, 0)
...
static inline r_obj* r_alloc_complex(r_ssize n) {
#if R_VERSION >= R_Version(4, 6, 0)
...
static inline r_obj* r_alloc_raw(r_ssize n) {
#if R_VERSION >= R_Version(4, 6, 0)
...
static inline r_obj* r_alloc_character(r_ssize n) {
#if R_VERSION >= R_Version(4, 6, 0)
...
static inline r_obj* r_alloc_list(r_ssize n) {
#if R_VERSION >= R_Version(4, 6, 0)
...
static inline r_obj* r_alloc_raw0(r_ssize n) {
...
static inline r_obj* r_chr(const char* c_string) {
    r_obj* out = KEEP(r_alloc_character(1));
    r_chr_poke(out, 0, r_str(c_string));
    FREE(1);
    return out;
}
static inline r_obj* r_list(r_obj* x) {
...
static inline r_obj* r_len(r_ssize x) {
    if (x > INT_MAX) {
        return r_dbl(x);
    } else {
        return r_int(x);
...
static inline r_obj* r_shared_lgl(bool x) {
    if (x) {
        return r_true;
    } else {
        return r_false;
...
static inline bool _r_has_correct_length(r_obj* x, r_ssize n) {
    return n < 0 || r_length(x) == n;
}
extern bool _r_is_finite(r_obj* x);

static inline bool _r_is_double(r_obj* x, r_ssize n, int finite) {
...
static inline bool _r_is_complex(r_obj* x, r_ssize n, int finite) {
...
static inline bool r_is_false(r_obj* x) {
    return r_is_bool(x) && !r_lgl_get(x, 0);
}
...
static inline r_complex r_as_complex(r_obj* x) {
    return r_arg_as_complex(x, "x");
}

static inline char r_arg_as_char(r_obj* x, const char* arg) {
...
static inline r_obj* r_vec_resize0(
    r_type type,
    r_obj* x,
    r_ssize new_size
) {
    switch (type) {
    case R_TYPE_logical:
        return r_lgl_resize(x, new_size);
    case R_TYPE_integer:
        return r_int_resize(x, new_size);
    case R_TYPE_double:
        return r_dbl_resize(x, new_size);
    case R_TYPE_complex:
        return r_cpl_resize(x, new_size);
    case R_TYPE_raw:
        return r_raw_resize(x, new_size);
    case R_TYPE_character:
        return r_chr_resize(x, new_size);
    case R_TYPE_list:
        return r_list_resize(x, new_size);
    default:
        r_stop_unimplemented_type(type);
    }
}
static inline r_obj* r_vec_resize(r_obj* x, r_ssize new_size) {
    return r_vec_resize0(r_typeof(x), x, new_size);
}

static inline r_obj* r_vec_n(r_type type, void* v_src, r_ssize n) {
    switch (type) {
    case R_TYPE_logical:
    case R_TYPE_integer:
    case R_TYPE_double:
...
static inline r_obj* r_lgl_n(int* v_src, r_ssize n) {
    return r_vec_n(R_TYPE_logical, v_src, n);
}
static inline r_obj* r_int_n(int* v_src, r_ssize n) {
    return r_vec_n(R_TYPE_integer, v_src, n);
}
static inline r_obj* r_dbl_n(int* v_src, r_ssize n) {
    return r_vec_n(R_TYPE_double, v_src, n);
}
static inline r_obj* r_cpl_n(int* v_src, r_ssize n) {
    return r_vec_n(R_TYPE_complex, v_src, n);
}
static inline r_obj* r_raw_n(int* v_src, r_ssize n) {
    return r_vec_n(R_TYPE_raw, v_src, n);
}

static inline r_obj* r_copy_in_raw(const void* src, size_t size) {
    r_obj* out = r_alloc_raw(size);
    r_memcpy(r_raw_begin(out), src, size);
    return out;
}

static inline void r_int_fill_iota0(int* p_x, int start, r_ssize n) {
    for (r_ssize i = 0; i < n; ++i) {
        p_x[i] = start++;
    }
}
static inline void r_int_fill_iota(r_obj* x) {
...
static inline int* r_int_unique(r_obj* x) {
    return r_int_unique0(r_int_begin(x), r_length(x));
}

static inline bool r_list_all_of(r_obj* x, bool (*predicate)(r_obj* x)) {
