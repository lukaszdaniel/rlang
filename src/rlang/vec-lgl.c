...
r_obj* r_lgl_which(r_obj* x, bool na_propagate) {
    ...
    if (na_propagate) {
        if (has_names) {
            // Mark `NA` locations with negative location for extracting names
            // later
            ...
            for (r_ssize i = 0, j = 0; i < n && j < out_n; ++i) {
                ...
                const bool missing = x_elt == r_globals.na_lgl;
                const int elt =
                    missing * r_globals.na_int + !missing * x_elt * (i + 1);
                v_out[j] = elt;
                j += (bool) elt;
            }
        }
    ...
    if (has_names) {
        ...
        if (na_propagate) {
            // `v_out` contains negative locations which tells you the location
            // of the name to extract while also serving as a signal of where
            // `NA`s should go in the finalized output
            ...
            // `v_out` doesn't contain `NA`, so we can use the locations
            // directly
            for (r_ssize i = 0; i < out_n; ++i) {
