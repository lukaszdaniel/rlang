...
static int arg_match(
    ...
    // Invariant: my_values[i:(len-1)] contains the values we haven't matched
    // yet
    for (; i < arg_len; ++i) {
        ...
        for (int j = i + 1; j < arg_len; ++j) {
            if (current_arg == v_my_values[j]) {
                ...
                // Replace matched value by the element that failed to match at
                // this iteration
...
static r_type arg_match_arg_nm_type(r_obj* arg_nm) {
