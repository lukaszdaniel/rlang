...
bool has_name_at(r_obj* x, r_ssize i) {
    r_obj* nms = r_names(x);
    return r_typeof(nms) == R_TYPE_character &&
...
static r_ssize list_squash(
    ...
    for (r_ssize i = 0; i != n_outer; ++i) {
        ...
        if (depth != 0 && is_spliceable(inner)) {
            inner = PROTECT(maybe_unbox(inner, is_spliceable));
            count =
                list_squash(info, inner, out, count, is_spliceable, depth - 1);
            UNPROTECT(1);
        } else {
...
r_obj* ffi_squash(r_obj* dots, r_obj* type, r_obj* pred, r_obj* depth_) {
    r_type kind = Rf_str2type(CHAR(r_chr_get(type, 0)));
